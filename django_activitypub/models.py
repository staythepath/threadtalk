...
from .services import (
    send_create_note_to_followers,
    send_update_note_to_followers,
    send_delete_note_to_followers,
    get_object,
    format_datetime,
    parse_datetime,
)
...
class NoteManager(TreeQuerySet):
    ...
    def upsert_remote(self, base_uri, obj):
        ...
        try:
            ...
            note = Note()
        note.remote_actor = RemoteActor.objects.get_or_create_with_url(full_obj['attributedTo'])[0]
        try:
            note.published_at = parse_datetime(full_obj['published'])
        except ValueError:
            # Fallback to handle if microseconds are causing issues
            note.published_at = timezone.datetime.strptime(full_obj['published'], '%Y-%m-%dT%H:%M:%SZ')
